...
// Função auxiliar para garantir que o usuário convidado exista no banco
async function getGuestUserId() {
  const db = await getDb();
  if (!db) return 1;

  try {
    // Tentar encontrar o usuário admin padrão ou o primeiro usuário
    const existingUser = await db.select().from(users).limit(1);
    if (existingUser.length > 0) {
      return existingUser[0].id;
    }

    // Se não houver nenhum usuário, criar um usuário convidado
    const result = await db.insert(users).values({
      openId: "guest-user",
      name: "Usuário Convidado",
      email: "guest@adegamufs.com",
      role: "admin",
    });
    
    // Em alguns drivers, o insert retorna o ID diretamente
    return (result as any)[0]?.insertId || 1;
  } catch (error) {
    console.error("Erro ao garantir usuário convidado:", error);
    return 1;
  }
}
...
export const appRouter = router({
  ...
  products: router({
    ...
    create: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        try {
          // Garantir um ID de usuário válido para a chave estrangeira createdBy
          const userId = ctx.user?.id || await getGuestUserId();
  ...
  // ============ EXPORT ============
  export: router({
    comparisonPDF: publicProcedure.query(async () => {
      try {
    ...
    comparisonExcel: publicProcedure.query(async () => {
      try {
        const comparison = await getPriceComparisonMatrix();
